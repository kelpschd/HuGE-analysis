...
```{r}
#| label: Load in required packages
DE_genes_h <- c("APOB", "CREB3L3") #update with any genes of interest, can be as long as you want!
```
...
# Query HuGeAMP database for phenotype and associated HuGE score

```{r}
#| label: Query HuGeAMP database for phenotype and associated HuGE score
index <- "huge" 
...
for (query in DE_genes_h) {
  ...
  huge_df <- bind_rows(as_tibble(huge_data$data), huge_df)
    
  huge_df <- huge_df %>%
    mutate(locus = paste0(chromosome, ":", start, "-", end))
...
ggplot(huge_df %>% filter(phenotype == "CHOL"), #you currently have to manually set the group you are interested in
...
# Query HuGeAMP database for top associated variants

```{r}
#| label: Query HuGeAMP database for top associated variants
index <- "huge" 
base_url_bioindex <- "https://bioindex.hugeamp.org"
endpoint_bioindex <- paste0("/api/bio/query/top-associations/?q=", index)
req_bioindex <- request(paste0(base_url_bioindex, endpoint_bioindex))
...
for (query in DE_genes_h) {
  ...
  variants <- req_bioindex %>%
    req_url_query(!!!query) %>%
    req_perform() %>%
    resp_body_string() %>%
    fromJSON()
  
  variants <- as_tibble(variants$data) %>%
    mutate(queried_gene = query)
  
  #write query to the dataframe
  variants_df <- bind_rows(as_tibble(variants), variants_df)
